...
class RTSim(object):
    ...
    def _ping_calculate(self):
        ...
        dir = np.asarray([np.cos(angle), np.sin(angle)])
        pos = self._position[0,0:2]
        ...
        for b in self._barriers:
            ...
            line[0:4] = [x, y, x+w, y]
            distance, point = self._intersection(pos, dir, line);
    ...
    def _intersection(self, pos, dir, line):
        distance = -1 # -ve distance => no intersection
        point = pos

        # lines are horizontal or vertical
        if (line[0] == line[2]) and dir[0]: # vertical
            t = (line[0] - pos[0]) / dir[0]
            if (t >= 0) and (t <= 1):
        ...
        if (line[1] == line[3]) and dir[1]: # horizontal
            t = (line[1] - pos[1]) / dir[1]
            if (t >= 0) and (t <= 1):
                s = (t * dir[0] - (line[0] - pos[0])) / (line[2] - line[0])
                if (s >= 0) and (s <= 1): # valid intersection
                    point = t * dir + pos
                    distance = t
